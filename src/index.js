// x402-server — the SELLER half of x402.
//
...
// the canonical verify → dispatch → settle order (work runs only after a valid
// payment; funds move only after the work succeeds). This is NOT the buyer side:
// it never signs or pays. Pair it with any x402 buyer-side client.
//
// Wire shapes follow the x402 v2 specification:
//   - the 402 challenge envelope + accept[] entries (one per network lane)
//   - the facilitator /verify + /settle bodies
//     ({ x402Version, paymentPayload, paymentRequirements })
//   - an optional platform fee split (clamped to 10%, carved out of the price,
//     never marking up the buyer)
//
...
// x402 v2 wire format version.
export const X402_VERSION = 2;

// CAIP-2 network IDs. Solana mainnet uses the truncated genesis-block hash;
// EVM lanes use eip155:<chainId>.
...
// No baked-in facilitator. A seller settles against its own facilitator, set via
// the X402_FACILITATOR_URL env var or the `facilitator`/`baseUrl` option. The
// default is an empty string — verify/settle require a URL to be supplied.
export const DEFAULT_FACILITATOR_URL = '';

// Hard ceiling on the optional platform fee — a guard so a fat-fingered config
// can never charge an absurd fee.
export const MAX_FEE_BPS = 1000; // 10%

// Canonical USDC mint/contract per lane. `asset: 'usdc'` (the default) resolves
// to these; pin explicit addresses with `asset: { solana, base }`.
const CANONICAL_USDC = {
	solana: 'EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v',
	base: '0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913',
};

// $THREE — an optional Solana SPL settlement token advertised alongside USDC.
// Solana-only (it's an SPL mint). `asset: 'three'` resolves to it; `acceptThree:
// true` advertises it next to USDC on the Solana lane. 6 decimals. USDC remains
// the default — $THREE is opt-in.
 ...
 * auth headers. `buildChallenge` needs no facilitator (it only constructs the
 * 402 envelope); `verifyPayment`/`settlePayment`/`paid` settle against the
 * facilitator from `facilitator`/`baseUrl` or the X402_FACILITATOR_URL env var.
 * Use this to reuse a facilitator override or a custom fetch across many routes.
 *
...
// A lazily-created shared client backs the default top-level functions, so
// `import { paid }` works without first calling createX402Server().
let shared = null;
...
function buildAccepts({ price, asset, payTo, network, feePayer, maxTimeoutSeconds, resourceUrl, acceptThree, threeAmount }) {
	...
	for (const lane of lanes) {
		...
		// When opted in, $THREE rides immediately after the USDC Solana accept, so a
		// wallet's token chooser surfaces both while a first-accept client still
		// settles USDC (USDC stays the default).
...
function buildAccept({ lane, amount, asset, payTo, feePayer, maxTimeoutSeconds, resourceUrl }) {
	...
	if (lane === 'solana') {
		// A Solana facilitator rejects with `missing_fee_payer` unless the accept
		// advertises the sponsor account that co-signs the SPL transfer.
		if (!feePayer) {
...
const nodeAdapter = {
	...
	challenge(ctx, body, _args, status = 402) {
		...
		// discovery crawler can read it off the header during a probe request.
